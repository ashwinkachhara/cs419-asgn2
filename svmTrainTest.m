...
model = svmtrain(labelToxicant(trainInd), Ktrain, '-t 4');
[predClass, acc, decVals] = svmpredict(labelToxicant(testInd), Ktest, model);

model = svmtrain(labelStatin(trainInd), Ktrain, '-t 4');
[predClass, acc, decVals] = svmpredict(labelStatin(testInd), Ktest, model);

model = svmtrain(labelAzole(trainInd), Ktrain, '-t 4');
[predClass, acc, decVals] = svmpredict(labelAzole(testInd), Ktest, model);

model = svmtrain(labelFibrate(trainInd), Ktrain, '-t 4');
[predClass, acc, decVals] = svmpredict(labelFibrate(testInd), Ktest, model);
