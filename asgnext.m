...
for i=1:numTrain,
    for j=1:numTrain,
        Ktrain(i,j+1) = prodkernel2(rho,mu(:,trainInd(i)),mu(:,trainInd(j)),variances(:,i),variances(:,j));
...
for i=1:numTest,
    for j=1:numTrain,
        Ktest(i,j+1) = prodkernel2(rho,mu(:,testInd(i)),mu(:,trainInd(j)),variances(:,testInd(i)),variances(:,trainInd(j)));
        strcat(num2str(i),',',num2str(j),',',num2str(Ktest(i,j+1)))
    end
end
Ktrain05 = Ktrain;
Ktest05 = Ktest;
save Ktest05 Ktest05
save Ktrain05 Ktrain05